...
export class SeoService {
  ...
  private setDefaultMetaTag(headTag: HTMLHeadElement, metaData: any): void {
    ...
    // description tag
    if (hasMetaTag('description'))
      updateMetaTag(
        'description',
        metaData?.default?.title ?? this.defaultDescription
      );
    else
      addMetaTag(
        'description',
        metaData?.default?.description ?? this.defaultDescription
      );
  }
  ...
  private setContentLanguageMataTag(
  ...
  private setOpenGraph(headTag: HTMLHeadElement, seoData: any): void {
    const addOpenGraphTags = () => {
      ...
      title.setAttribute(
        'content',
        seoData?.default?.title ?? this.defaultTitle
      );
      ...
      description.setAttribute(
        'content',
        seoData?.default?.description ?? this.defaultDescription
      );
      ...
      image.setAttribute(
        ...
        `${window.location.origin}${
          seoData?.default?.image ??
          '/assets/images/illustrations/dark/banner.svg'
        }`
      );
    ...
    const updateTags = () => {
      ...
      for (let i = 0, l = metas.length; i < l; i++) {
        ...
        for (let j = 0, le = attributes.length; j < le; j++) {
          if (
            attributes[j].name === 'property' &&
            attributes[j].value === 'og:type'
          ) {
            metas[i].content = seoData?.default?.type ?? 'website';
          } else if (
            attributes[j].name === 'property' &&
            attributes[j].value === 'og:title'
          ) {
            metas[i].content = seoData?.default?.title ?? this.defaultTitle;
          } else if (
            attributes[j].name === 'property' &&
            attributes[j].value === 'og:description'
          ) {
            metas[i].content =
              seoData?.default?.description ?? this.defaultDescription;
          } else if (
            attributes[j].name === 'property' &&
            attributes[j].value === 'og:site_name'
          ) {
            metas[i].content = SITE_NAME;
          } else if (
            attributes[j].name === 'property' &&
            attributes[j].value === 'og:url'
          ) {
            metas[
              i
            ...
            attributes[j].name === 'property' &&
            attributes[j].value === 'og:image'
          ) {
            metas[i].content = `${window.location.origin}${
              seoData?.default?.image ??
              '/assets/images/illustrations/dark/banner.svg'
            }`;
          }
  ...
  private setTwitterCards(headTag: HTMLHeadElement, seoData: any): void {
    const addTwitterCards = () => {
      ...
      creator.setAttribute(
        'content',
        seoData?.default?.siteUserName ?? 'Sanjib Kumar Mandal'
      );
    ...
    const updateTwitterCards = () => {
      ...
      for (let i = 0, l = metas.length; i < l; i++) {
        ...
        for (let j = 0, le = attributes.length; j < le; j++) {
          if (
            attributes[j].name === 'property' &&
            attributes[j].value === 'twitter:title'
          ) {
            metas[i].content = seoData?.default?.title ?? this.defaultTitle;
          } else if (
            attributes[j].name === 'property' &&
            attributes[j].value === 'twitter:description'
          ) {
            metas[i].content =
              seoData?.default?.description ?? this.defaultDescription;
          } else if (
            attributes[j].name === 'property' &&
            attributes[j].value === 'twitter:site'
          ) {
            metas[i].content = 'Koniat';
          } else if (
            attributes[j].name === 'property' &&
            attributes[j].value === 'twitter:image'
          ) {
            metas[i].content = `${window.location.origin}${
              seoData?.default?.image ??
              '/assets/images/illustrations/dark/banner.svg'
            ...
            attributes[j].name === 'property' &&
            attributes[j].value === 'twitter:creator'
          ) {
            metas[i].content =
              seoData?.default?.siteUserName ?? 'Sanjib Kumar Mandal';
          }
