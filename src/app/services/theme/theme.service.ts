...
export class ThemeService {
  ...
  get themeState$() { return this.currentThemeObserver.pipe(filter(el => el != null)) }
  ...
  private applyTheme() {
    ...
    this.themeState$.subscribe((theme) => {
      if(!hasThemeMeta) {
        ...
        meta.content = theme === Theme.Dark ? ThemeColors[Theme.Dark] : ThemeColors[Theme.Light];
        head.appendChild(meta);
        const msapplicationmeta = document.createElement('meta');
        msapplicationmeta.name = 'msapplication-navbutton-color';
        msapplicationmeta.content = theme === Theme.Dark ? ThemeColors[Theme.Dark] : ThemeColors[Theme.Light];
        head.appendChild(msapplicationmeta);
        const applemeta = document.createElement('meta');
        applemeta.name = 'apple-mobile-web-app-status-bar-style';
        applemeta.content = theme === Theme.Dark ? ThemeColors[Theme.Dark] : ThemeColors[Theme.Light];
        ...
        for (let index = 0, l = meta.length; index < l; index++) {
          if(meta[index].name === 'theme-color') {
            meta[index].content = theme === Theme.Dark ? ThemeColors[Theme.Dark] : ThemeColors[Theme.Light];
          } else if(meta[index].name === 'msapplication-navbutton-color') {
            meta[index].content = theme === Theme.Dark ? ThemeColors[Theme.Dark] : ThemeColors[Theme.Light];
          } else if(meta[index].name === 'apple-mobile-web-app-status-bar-style') {
            meta[index].content = theme === Theme.Dark ? ThemeColors[Theme.Dark] : ThemeColors[Theme.Light];
          }
        }
      }
